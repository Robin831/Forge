...
// DefaultReviewer is the GitHub reviewer handle for GitHub Copilot PR reviews.
const DefaultReviewer = "copilot-pull-request-reviewer"

// RequestReReview requests a re-review from the given reviewer on a PR.
// This is used after pushing review fixes so the reviewer (e.g. GitHub Copilot)
// is notified to re-examine the PR.
func RequestReReview(ctx context.Context, worktreePath string, prNumber int, reviewer string) error {
	if reviewer == "" {
	...
	log.Printf("[ghpr] Requested re-review from %s on PR #%d", reviewer, prNumber)
	return nil
}
