...
// ── Update overlay state machine tests ──────────────────────────────────────

// TestUpdateOverlayEscDuringScanClosesOverlay verifies that pressing Esc while the
// scan is in progress closes the overlay immediately.
func TestUpdateOverlayEscDuringScanClosesOverlay(t *testing.T) {
	m := NewModel(nil)
	m.UpdateAnvils = []depupdate.Anvil{{Name: "test", Path: "/tmp/test"}}

	// Start the scan — this sets showUpdateOverlay=true, updateScanning=true.
	_ = m.openUpdateOverlay()

	if !m.showUpdateOverlay {
		t.Fatal("expected showUpdateOverlay to be true after openUpdateOverlay")
	}
	if !m.updateScanning {
		t.Fatal("expected updateScanning to be true after openUpdateOverlay")
	}
	...
	if model.showUpdateOverlay {
		t.Error("expected showUpdateOverlay to be false after Esc")
	}
	if model.updateScanning {
		t.Error("expected updateScanning to be false after Esc")
	}
}

// TestUpdateOverlayLateScandoneIgnoredAfterClose verifies that an updateScanDoneMsg
// arriving after the overlay is closed (mismatched generation) is discarded without
// reopening the overlay or populating form state.
func TestUpdateOverlayLateScandoneIgnoredAfterClose(t *testing.T) {
	m := NewModel(nil)
	m.UpdateAnvils = []depupdate.Anvil{{Name: "test", Path: "/tmp/test"}}

	// Simulate: open overlay, capture generation, then close before scan completes.
	_ = m.openUpdateOverlay()
	gen := m.updateScanGeneration
	m.closeUpdateOverlay() // user pressed Esc

	// Deliver a scan result with the now-stale generation.
	reports := []depupdate.AnvilReport{
		{Anvil: depupdate.Anvil{Name: "test"}, Groups: []depupdate.UpdateGroup{{Name: "g1", Kind: "patch"}}},
	}
	m2, _ := m.Update(updateScanDoneMsg{reports: reports, generation: gen})
	model := m2.(*Model)

	if model.showUpdateOverlay {
		t.Error("expected overlay to stay closed for stale scan result")
	}
	if model.updateForm != nil {
		t.Error("expected updateForm to be nil for stale scan result")
	}
}

// TestUpdateOverlayMouseBlockedWhenOpen verifies that mouse wheel events are
// consumed (not forwarded to panels) while the update overlay is open.
func TestUpdateOverlayMouseBlockedWhenOpen(t *testing.T) {
	m := NewModel(nil)
	m.UpdateAnvils = []depupdate.Anvil{{Name: "test", Path: "/tmp/test"}}
	m.width = 120
	m.height = 40
	...
	if model.focused != initialFocused {
		t.Errorf("expected panel focus unchanged during update overlay, got %v", model.focused)
	}
	if cmd != nil {
		t.Error("expected nil cmd for mouse event while update overlay is open")
	}
}

// TestUpdateOverlayNoAnvilsShowsToast verifies that opening the overlay when no
// anvils are configured adds an error toast rather than showing the overlay.
func TestUpdateOverlayNoAnvilsShowsToast(t *testing.T) {
	m := NewModel(nil)
	// No anvils configured.

	_ = m.openUpdateOverlay()

	if m.showUpdateOverlay {
		t.Error("expected overlay to stay hidden when no anvils configured")
	}
	if len(m.toasts) == 0 {
		t.Error("expected an error toast when no anvils configured")
	}
}

// TestUpdateOverlayScanErrorClosesOverlay verifies that a failed scan (top-level
// error) closes the overlay and adds a toast.
func TestUpdateOverlayScanErrorClosesOverlay(t *testing.T) {
	m := NewModel(nil)
	m.UpdateAnvils = []depupdate.Anvil{{Name: "test", Path: "/tmp/test"}}
	_ = m.openUpdateOverlay()

	m2, _ := m.Update(updateScanDoneMsg{
		err:        errors.New("network unreachable"),
		generation: m.updateScanGeneration,
	})
	model := m2.(*Model)

	if model.showUpdateOverlay {
		t.Error("expected overlay to close on scan error")
	}
	if len(model.toasts) == 0 {
		t.Error("expected an error toast after scan error")
	}
}
