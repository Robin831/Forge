...
func TestReconcileOpenPRs_RequiresPerInstanceForgeManagedMarker(t *testing.T) {
	...
	mock := &mockVCSProvider{
		openPRs: []vcs.OpenPR{
			...
			// marker. Must be adopted as bellows-managed. It targets an epic
			// branch rather than main, which is the case the persisted
			// BaseBranch exists for: a Crucible child re-registered here with
			// an empty base has its changed-file gating computed against
			// origin/main, and the rebase path force-pushes onto main and
			// destroys its head.
			{
				Number:     4040,
				Title:      "forge: Forge-real",
				Branch:     "forge/Forge-real",
				BaseBranch: "feature/Forge-parent",
				Body:       "## Changes\n\nDid the work.\n\n---\nBead: Forge-real | Branch: forge/Forge-real\n" + vcs.MarkerForID(myID),
			},
			// 5050: purely external PR with no bead reference at all, and no
			// base branch reported — the degraded path that must still store
	...
	assert.Equal(t, "feature/Forge-parent", mine.BaseBranch,
		"reconcile must persist the base branch the platform reports — an empty one is read as origin/main "+
			"by temper.ResolveBaseRef and force-pushed by the rebase path, which destroys a Crucible child's head")
	...
	assert.Equal(t, "", random.BaseBranch,
		"a provider that reports no base branch stores none: the origin/main fallback stays the "+
			"degraded path downstream rather than a guess written into the row")
