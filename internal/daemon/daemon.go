...
type Daemon struct {
	...
	queueMeta   map[string]queueSideMeta
}

// queueSideMeta carries the bd-emitted bead metadata that rides beside the
...
func New(cfg *config.Config, configPath string) (*Daemon, error) {
	...
	d := &Daemon{
		...
		queueMeta:             make(map[string]queueSideMeta),
...
func (d *Daemon) Run(ctx context.Context) error {
	...
	if d.config().Settings.IsSmelterEnabled() {
		d.smelterWorker = smelter.New(
			...
			smelter.WithOverlapThreshold(func() float64 {
				return d.config().Settings.Warden.ResolvedOverlapThreshold()
			}),
...
func (d *Daemon) pollAndDispatch(ctx context.Context, fullPoll bool) {
	...
	if len(succeededAnvils) > 0 {
		...
		// the cache exactly.
		stamps := make(map[string]queueSideMeta)
		for _, b := range mergedForCache {
			...
			stamps[b.Anvil+"/"+b.ID] = queueSideMeta{CreatedAt: b.CreatedAt, UpdatedAt: b.UpdatedAt, CreatedBy: b.CreatedBy}
		}
		...
		if err := d.db.ReplaceQueueCacheForAnvils(succeededAnvils, dedupeCacheItems(cacheItems)); err != nil {
			...
			d.replaceQueueMeta(succeededSet, stamps)
		}
	}
...
func (d *Daemon) dispatchBead(ctx context.Context, bead poller.Bead, anvilCfg config.AnvilConfig, claimWorkerID string, ctrl *controlHandle, resume *pipeline.ResumeSession, costReservation uint64) {
	...
	// An opted-in parent with no children left has nothing to orchestrate and
	// runs as an ordinary bead rather than creating an empty feature branch.
	//
	// "No children left" is not what an empty bead.Blocks says, though: pollAnvil
	// reconstructs Blocks from the beads in *this* poll batch, so a child that
	// exists but is blocked on an unrelated dependency simply is not there.
	// Reading that as "nothing to orchestrate" merges the parent to main and
	// closes it while its opt-in label lives on — and the first child to become
	// ready is then stamped with a branch nobody will ever create, hard-failing
	// in worktree.Create every dispatch until its circuit breaker burns. So ask
	...
	if poller.IsOrchestratedParent(bead) && !crucible.IsCrucibleCandidate(bead) {
		...
		case parentDefer:
			...
			d.logger.Warn("cannot determine whether an orchestrated parent still has children, deferring dispatch",
				"bead", bead.ID, "anvil", bead.Anvil, "error", childErr)
			if err := d.db.UpdateWorkerStatus(claimWorkerID, state.WorkerFailed); err != nil {
				d.logger.Warn("failed to mark pending worker failed after deferring an orchestrated parent",
...
func (d *Daemon) handleIPC(cmd ipc.Command) ipc.Response {
	...
	case "queue":
		...
		for _, it := range items {
			...
			ts := d.lookupQueueMeta(it.Anvil, it.BeadID)
...
// replaceQueueMeta refreshes the in-memory side-map for the given set of
// successfully-polled anvils. Entries belonging to other anvils are kept
// verbatim, mirroring ReplaceQueueCacheForAnvils so a failed anvil poll does
// not wipe its last-known metadata. `fresh` is the full set of new entries
// from this poll cycle (its keys must use the "anvil/beadID" format).
func (d *Daemon) replaceQueueMeta(succeeded map[string]struct{}, fresh map[string]queueSideMeta) {
	d.queueMetaMu.Lock()
	defer d.queueMetaMu.Unlock()
	next := make(map[string]queueSideMeta, len(fresh)+len(d.queueMeta))
	// Keep entries from anvils that did not poll successfully this cycle so
	// their metadata remains available alongside their cached queue rows.
	for k, v := range d.queueMeta {
...
// lookupQueueMeta returns the CreatedAt/UpdatedAt/CreatedBy trio recorded
// for the given (anvil, beadID) during the most recent poll. Callers receive a
// zero-valued queueSideMeta (empty strings) when no entry is present.
func (d *Daemon) lookupQueueMeta(anvil, beadID string) queueSideMeta {
	d.queueMetaMu.RLock()
	defer d.queueMetaMu.RUnlock()
	return d.queueMeta[anvil+"/"+beadID]
}
