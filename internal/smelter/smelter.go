...
func (s *Smelter) flushAnvil(ctx context.Context, anvilName, anvilPath string, rules []state.PendingRule) error {
	...
	if added == 0 {
		// All rules were duplicates (already in the file). Delete from DB and move on.
		log.Printf("[smelter] All %d pending rule(s) for %s were duplicates — cleaning up", len(rules), anvilName)
...
func (s *Smelter) ensurePR(ctx context.Context, wtPath, anvilName, branch string, ruleCount int) error {
	...
	cmd := executil.HideWindow(exec.CommandContext(ctx, "gh", "pr", "create",
		"--title", prTitle,
		"--body", body,
		"--base", "main",
		"--head", branch,
