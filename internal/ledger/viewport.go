...
// viewStart tracks the first visible item in the viewport.
...
// height (in item slots). Call this during rendering when the viewport height
// is known.
...
// items visible in a viewport of the given height.
func (v *scrollViewport) VisibleRange(viewHeight, total int) (start, end int) {
	if viewHeight <= 0 {
		viewHeight = 1
	}
	start = v.viewStart
	end = min(start+viewHeight, total)
