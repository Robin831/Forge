...
// recordSessionCost persists one completed quench session's token usage into the
// three cost tables, through the same cost.Record fan-out the pipeline stages
// use. Until this existed, quench spend reached the provider quota and the
...
func recordSessionCost(db *state.DB, beadID, anvil, provName string, r *smith.Result) {
	...
	if err := cost.Record(db, provName, beadID, anvil, r.Usage()); err != nil {
		log.Printf("[quench] bead=%s: cost write failed: %v", beadID, err)
	}
}
	...
	for pi := 0; pi < len(providers); pi++ {
		...
		if p.WorkerID != "" && p.DB != nil {
			...
			if err := p.DB.UpdateWorkerPID(p.WorkerID, process.PID); err != nil {
				log.Printf("[quench] PR #%d: failed to update worker PID: %v", p.PRNumber, err)
			}
	...
	for attempt := 1; attempt <= MaxAttempts; attempt++ {
		...
		for pi := activeProviderIdx; pi < len(providers); pi++ {
			...
			if p.WorkerID != "" && p.DB != nil {
				...
				if err := p.DB.UpdateWorkerPID(p.WorkerID, process.PID); err != nil {
					log.Printf("[quench] PR #%d: failed to update worker PID for worker %s: %v",
