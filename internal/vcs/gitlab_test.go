...
// TestGitLabCreatePR_AlreadyExistsError verifies that CreatePR errors containing
// "already exists" are wrapped with ErrPRAlreadyExists so callers can use
// errors.Is instead of fragile string matching.
func TestGitLabCreatePR_AlreadyExistsError(t *testing.T) {
	tests := []struct {
		name         string
		stderrStr    string
		wantSentinel bool
	}{
		{"already exists message", "merge request already exists for source branch", true},
		{"unrelated error", "authentication failed", false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Simulate the error wrapping logic from GitLabProvider.CreatePR
			var resultErr error
			if strings.Contains(tt.stderrStr, "already exists") {
				resultErr = fmt.Errorf("glab mr create: %w: %s", ErrPRAlreadyExists, tt.stderrStr)
			} else {
				resultErr = fmt.Errorf("glab mr create failed: %s", tt.stderrStr)
			}
