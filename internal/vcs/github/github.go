...
func (p *Provider) CreatePR(ctx context.Context, params vcs.CreateParams) (*vcs.PR, error) {
	...
	// merely reference a bead ID — and from PRs created by other Forge
	// instances pointing at the same anvil.
	params.Body = vcs.EnsureForgeManagedMarker(params.Body)
...
func buildDefaultBody(p vcs.CreateParams) string {
	...
	b.WriteString(vcs.MarkerForID(vcs.ForgeID()))
