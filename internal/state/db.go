...
CREATE TABLE IF NOT EXISTS depcheck_consolidated_beads (
    anvil      TEXT PRIMARY KEY,
    bead_id    TEXT NOT NULL DEFAULT '',
    updated_at TEXT NOT NULL DEFAULT ''
);
...
// rather than created by a Forge pipeline.
func (p *PR) IsExternal() bool {
	return strings.HasPrefix(p.BeadID, "ext-")
}
...
// UpdatePRBellowsDetached sets the bellows_detached flag for a PR.
// When true, bellows leaves this PR alone: no events, no automatic lifecycle
// workers. It is written only by an explicit operator action, never by the
// reconcile loop, so detaching a PR outlives the managed-flag rewrites that
// reevaluateTrackedPR performs on every cycle.
func (db *DB) UpdatePRBellowsDetached(id int, detached bool) error {
	_, err := db.conn.Exec(`UPDATE prs SET bellows_detached = ? WHERE id = ?`, boolToInt(detached), id)
	return err
}

// UpdatePRTitle sets the title for a PR record.
func (db *DB) UpdatePRTitle(id int, title string) error {
	_, err := db.conn.Exec(`UPDATE prs SET title = ? WHERE id = ?`, title, id)
	return err
}
...
func (db *DB) OpenPRsWithDetail() ([]OpenPRDetail, error) {
	rows, err := db.conn.Query(
		`SELECT p.id, p.number, p.anvil, p.bead_id, p.branch, p.status,
		        COALESCE(NULLIF(q.title, ''), NULLIF(w.title, ''), NULLIF(p.title, ''), '') AS title,
...
const (
	...
	// EventDispatchRecoveredStrandedBranch fires when the pre-dispatch remote
	// check finds a stranded origin/forge/<bead-id> with a changelog fragment
	// for the bead reachable from the branch tip (a completion signal) and no
