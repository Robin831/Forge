...
// Assay resolves each pass from them (assay.Config.providersFor): anvil
// "assay.<pass>", anvil "assay", global "assay.<pass>", global "assay", then
// the legacy assay.triage_provider / assay.review_provider keys, then the
// provider defaults — never smith_providers or settings.providers.
...
var managedConfigKeys = []configKeyDef{
	...
	},
	{
		...
		Description: "Per-stage provider overrides keyed by pipeline stage (smith, warden, schematic, cifix, reviewfix, assay, and the per-pass assay.triage, assay.logic, assay.security, assay.conventions, assay.tests-missing, assay.repo-specific). Each value is an ordered provider chain. Assay resolves each pass from anvil assay.<pass>, anvil assay, global assay.<pass>, global assay, then the legacy assay.triage_provider / assay.review_provider keys, then the provider defaults; it never inherits smith_providers or providers. Each Assay pass starts on the head of its own chain and moves to the next entry only when rate limited.",
		...
		// config per spawn. The assay/assay.* keys are read the same way: the
		// daemon builds each review's engine config from the live config
		// (assay.ForAnvil) when the review starts.
		HotReloadable: true,
	...
	{
		Key:         "preview_proxy_base",
		Type:        typeString,
		Area:        "Previews",
		Label:       "Preview proxy base domain",
		Description: "DNS suffix previews are addressed under when Forge fronts them by hostname: a bead answers on <label>.<base> and one of its services on <label>--<service>.<base>. Needs a wildcard record pointing at this box. Empty keeps preview links on host:port.",
		Default:     "",
		value:       func(s config.SettingsConfig) any { return s.PreviewProxyBase },
	},
...
var managedAnvilKeys = []anvilKeyDef{
	...
	{Key: "stage_providers", Type: typeProviderMap, Options: providerStages, Instant: true,
		Label: "Per-stage providers", Description: "Per-anvil override of the global per-stage provider chains (smith, warden, schematic, cifix, reviewfix, assay and the per-pass assay.* keys). An anvil assay or assay.<pass> key outranks every global one. Each Assay pass falls back down its own chain only on a rate limit."},
