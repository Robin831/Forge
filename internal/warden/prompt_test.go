...
func TestBuildReviewPrompt_ContainsBeadMetadata(t *testing.T) {
	prompt := buildReviewPrompt("Forge-123", "Fix the login bug", "Users cannot log in after password reset.", "diff content", "")
...
func TestBuildReviewPrompt_OmitsDescriptionWhenEmpty(t *testing.T) {
	prompt := buildReviewPrompt("Forge-456", "Refactor config loader", "", "diff content", "")
...
func TestBuildReviewPrompt_ContainsScopeAlignmentCheck(t *testing.T) {
	prompt := buildReviewPrompt("Forge-001", "Some task", "Do something useful.", "diff content", "")
...
func TestBuildReviewPrompt_ContainsDiff(t *testing.T) {
	diff := "--- a/main.go\n+++ b/main.go\n@@ -1 +1 @@\n-old\n+new"
	prompt := buildReviewPrompt("Forge-002", "Title", "Desc", diff, "")
