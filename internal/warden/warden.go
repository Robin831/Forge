...
// wardenDiffMaxBytes limits the diff size passed to the Warden review. Sized
// to cover real EF Core migration diffs (~138KB observed) with headroom for
// larger schemas while still keeping the Warden prompt comfortably within a
// single-turn budget. Auto-generated files are filtered out before this cap
// is applied, so most diffs stay well under the limit.
const wardenDiffMaxBytes = 250000

// autoGeneratedDiffPatterns is the default list of glob patterns (doublestar
// syntax) for auto-generated files whose diff hunks are dropped before the
// Warden review. These snapshots mirror the entire model and are not
// human-reviewable; leaving them in the diff either triggers truncation or
// leads the reviewer to misread the bulk as unrelated schema changes and
// reject on scope drift.
var autoGeneratedDiffPatterns = []string{
...
func buildReviewPrompt(beadID, beadTitle, beadDescription, diff, anvilPath, priorFeedback string) string {
	...
	filteredDiff, elided := filterAutoGeneratedFiles(diff, autoGeneratedDiffPatterns)
	filteredDiff = truncateDiff(filteredDiff, wardenDiffMaxBytes)
...
// filterAutoGeneratedFiles drops git-diff hunks whose file path matches any of
// the given doublestar glob patterns. Returns the filtered diff and the list
// of elided file paths (in the order they appeared).
//
...
// unchanged.
func filterAutoGeneratedFiles(diff string, patterns []string) (string, []string) {
	...
	for len(remaining) > 0 {
		...
		path := parseDiffGitPath(headerLine)
...
// parseDiffGitPath extracts the b-side file path from a git diff header line
// of the form "diff --git a/<path> b/<path>". Returns "" if the header cannot
// be parsed. The b-side path is preferred because it reflects the file's
...
// patterns. Invalid patterns are skipped silently (they never match) so a
// bad entry cannot accidentally elide every file.
