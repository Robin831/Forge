...
assay:
  ...
  run_cost_estimate_usd: 5.0 # floor under the estimate reserved for a review that is
                             # running but has not recorded its cost yet, so the cap
                             # above accounts for work in flight (default 5.0)
...
| `run_cost_estimate_usd` | float\|null | `5.0` | Floor under the estimate held for one Assay review that has been admitted but has not yet recorded its cost. The effective estimate is the larger of this floor and the rolling mean of the last 20 recorded runs (seeded from `assay_runs` at daemon start, folded forward as each run finishes), so the floor is what covers a cold ledger — a daemon that has just started and whose first review has not finished. Deliberately conservative: under-estimating re-admits the overrun the reservation exists to stop, while over-estimating only stops reviews slightly early against a cap that is itself a safety limit. `0` means no floor (estimate from recorded runs alone); negative is rejected. Only meaningful alongside `daily_cost_limit_usd`. |
