...
| `stage_providers` | map[string][]string | `{}` | | Per-stage provider overrides. Keys: `smith`, `warden`, `schematic`, `cifix`, `reviewfix`, `assay`, and the per-pass Assay keys `assay.triage`, `assay.logic`, `assay.security`, `assay.conventions`, `assay.tests-missing`, `assay.repo-specific` (dotted keys are written verbatim, e.g. `assay.conventions: [gemini]`; an unknown key or an empty chain is rejected by the settings page). Each value is a provider chain in the same syntax as `providers`. Fallback: `stage_providers[stage]` → `smith_providers` (smith/warden/schematic) → `providers` → defaults. **Assay passes** resolve per pass and never inherit `smith_providers` or `providers`: anvil `stage_providers["assay.<pass>"]` → anvil `stage_providers["assay"]` → global `assay.<pass>` → global `assay` → the legacy `assay` block (`triage_provider`/`triage_model` for triage, falling back to `review_provider`/`review_model`; `review_provider`/`review_model` for the deep passes; anvil overlay over global) → the head of the provider defaults (Claude). An anvil `assay` key therefore beats a global `assay.logic`. **An assay chain is a per-pass rate-limit fallback:** each pass starts on the head of its own chain, and moves to the next entry only when the current one is rate limited — the turn-budget retry and the strict-JSON re-prompt run on one provider first. An auth failure never fails over (the pass fails and the chain stops), and a failover on one pass never changes the provider another pass runs on. The provider kind and model each pass actually ran on are recorded on the run's pass rows (`assay_runs.pass_findings`, keys `provider`/`model`/`failed_over`), and the `Assay review completed` log line carries `model=<id>` on every pass, plus `provider=<kind>` on a pass that failed over. `forge doctor` names each pass's fallbacks and warns when a fallback's binary is missing. When an Assay-enabled anvil sets both a legacy key and an assay stage key the daemon logs one WARN naming which one decides each pass, and `forge doctor` prints every pass's resolved provider and the step that supplied it, per anvil. |
...
| `github_no_close_labels` | string list | `["innmeldt"]` | `["innmeldt", "do-not-close"]` | Issue labels that mark a GitHub issue as must-not-auto-close. When a bead's `external_ref` issue carries one of these labels, the PR body references it with `Refs` instead of `Closes`, so merging the PR leaves the issue open until its reporter verifies the fix. The label lookup runs at PR-creation time via `gh issue view`; on lookup failure the issue is treated as closable. PR bodies always derive their issue reference from the worked bead's own `external_ref` (in the cross-repo-safe `owner/repo#N` form for full issue URLs), demote any other closing reference to `Refs`, and add a `Refs` line for the issue named by a wicket `Source:` line in the bead description. |
...
settings:
  warden:
    ...
    archive_after_days: 90     # Smelter staleness sweep AGE threshold; 0 = default 90; negative disables the sweep
    inactive_after_days: 90    # ...and its INACTIVITY threshold; a rule is archived only past BOTH; 0 = use archive_after_days
    allow_archive_terminus: false  # archive an aged, inactive rule even when archived rules were merged into it
...
| `warden.archive_after_days` | int | `90` | Staleness threshold (days) used by the Smelter's Pass 2 sweep: a rule older than this with no recent source activity is archived with reason `stale`. Age is counted from the rule's `merged_at` where consolidation stamped one and from `added` otherwise — a merged rule's `added` is inherited from the rules it folded, and since consolidation exists to collapse *old* near-duplicates, counting from it made every such survivor stale on the day it was created. `0` (or omit) uses the default of `90`; a negative value disables the pass ("never archive"). The default was 180 and is 90 because at 180 the sweep was inert on the files it exists to bound: on one anvil whose rules file had reached four figures, the rules added in 2026-04, -05 and -06 were all under 180 days old and Pass 2 archived nothing while the file kept growing. A convention still being violated keeps producing review comments, which refresh a rule's source activity and keep it out of the sweep, so 90 days removes a rule nothing has raised in a quarter — and the archive keeps it, so lengthening the threshold is enough to want it back. |
| `warden.inactive_after_days` | int | falls back to `archive_after_days` | The second half of the staleness sweep: how long a rule must have gone **unused** — unemitted into any review checklist and uncredited with any finding (`last_emitted`/`last_finding`, see Warden rule usage telemetry) — before it is archived. A rule is retired only when it has crossed **both** this and `archive_after_days`. The two are separate knobs because they measure different things: age is a fact about the distillation session that produced the rule, inactivity a fact about whether anything has wanted it since — and on age alone a rule the review selection puts in front of a reviewer every week is retired for being old. `0` (or omit) uses the resolved `archive_after_days`, which is exactly the behaviour of a deployment that never configured it. Unlike every other knob here a **negative value does not disable** this half, because switching it off *is* the defect; a deployment that wants retirement on age alone sets this to `1`. |
| `warden.allow_archive_terminus` | bool | `false` | Lets the staleness sweep archive a rule that other, **archived** rules were merged into (a supersession terminus). Off by default: such a rule carries the merged content of every rule behind it, each of those is already archived, so retiring it retires the whole chain with nothing left on the active file to say what went. The rules the guard holds back are named in the flush log, the Smelter commit body and the batch PR body, so an operator turns this on having read which ones. `forge warden consolidate --force` is the same override for one off-cycle run. |
...
assay:
  ...
  run_cost_estimate_usd: 1.0 # floor under the estimate reserved for a review that is
                             # running but has not recorded its cost yet, so the cap
                             # above accounts for work in flight (default 1.0, and
                             # capped by daily_cost_limit_usd however it is set)
  max_runs: 2                # executed reviews per PR; <=0 = no cap (default 2)
  max_cost_per_pass_usd: 3.0 # stop a single review pass session once its estimated
                             # spend reaches this; 0 = no ceiling (default 3.0)
...
| `daily_cost_limit_usd` | float\|null | `5.0` | Per-calendar-day USD cap on Assay review spend, measured against `assay_runs` (not the main `daily_costs` ledger). Prevents a runaway PR loop from silently burning quota. Set `0` for no cap. The gate projects **recorded spend + the estimates held for reviews already running + one estimate for the review being admitted** against the cap (see `run_cost_estimate_usd`): a run only lands in `assay_runs` when it ends, so a gate reading recorded spend alone admits reviews on a budget the reviews already in flight have spent. Two refusals come out of this and they behave differently. *Exhausted for the day* — the recorded spend has reached the cap, or what is left of it will not cover one review — skips the review and counts its head as reviewed for merge readiness, since nothing will change before the budget resets at UTC midnight. *Deferred* — the budget covers a review but the ones already running have it spoken for — leaves the head unreviewed and **not** released to merge readiness, because it clears within minutes. Bellows logs each and records an `assay_skipped` event once per PR head. |
| `run_cost_estimate_usd` | float\|null | `1.0` | Floor under the estimate held for one Assay review that has been admitted but has not yet recorded its cost. The effective estimate is the larger of this floor and the rolling mean of the last 20 recorded runs (seeded from `assay_runs` at daemon start, folded forward as each run finishes), so the floor is what covers a cold ledger — a daemon that has just started and whose first review has not finished. Whatever it resolves to, the estimate is then **bounded above by `daily_cost_limit_usd`**: an estimate larger than the whole cap fails the projection even on a day that has spent nothing, so every review would be refused as *exhausted for the day* and every head released to merge readiness unreviewed — a cap is a spend limit, never a switch that turns review off while merges keep running. The default is a fifth of the default cap for the same reason: at floor >= cap an unconfigured deployment admits one review and then releases the rest of the day's PRs. Raise the cap rather than the floor if your reviews genuinely cost more. `0` means no floor (estimate from recorded runs alone); negative is rejected. Only meaningful alongside `daily_cost_limit_usd`. |
...
| `max_cost_per_pass_usd` | float\|null | `3.0` | USD ceiling on a single review pass **session**. Assay prices each turn from the usage the provider streams (one API message, however many content-block events carry it) and stops the session the moment the running total reaches this value, reporting the pass as failed with `error_max_cost` — never as a short but successful review. `0` disables the ceiling. It bounds a session, not a run: a pass that takes a strict-JSON re-prompt or a turn-budget retry starts each of those with a fresh budget, and a stopped pass costs the run its coverage (the run comes back `partial`, naming the pass) rather than the run itself. It is a runaway brake and should sit well above a normal pass — raise it on deployments running a premium model over large diffs rather than have ordinary passes clipped. The ceiling only applies where the provider streams per-turn usage (Claude); backends that do not are never stopped on cost. The default moved $1.50 -> $3.00 in 2026-08 after the lower value was measured stopping four of one run's five deep passes at $1.51-$1.58 — a clip point rather than a runaway brake; see [assay-turn-budget.md](assay-turn-budget.md) for the distribution and why it is set by clearing the censoring point rather than by a percentile. Note the figure compared against this ceiling is Assay's own in-flight estimate, which prices input and cache only and so reads roughly 1.6x under the provider's final billed figure — set it in the estimate's unit, not in billed dollars. |
...
| `triage_provider` | string | `""` (Claude) | Provider spec (same syntax as `settings.providers`) for the cheap triage pass. Empty defaults to Claude. Legacy: any `stage_providers` `assay`/`assay.triage` key in scope outranks it (see `stage_providers`). |
| `review_provider` | string | `""` (Claude) | Provider spec for the five deep passes. Empty defaults to Claude. Legacy: any `stage_providers` `assay`/`assay.<pass>` key in scope outranks it for that pass. |
...
| `FORGE_SETTINGS_GITHUB_NO_CLOSE_LABELS` | `settings.github_no_close_labels` |
