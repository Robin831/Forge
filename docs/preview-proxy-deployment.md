...
| Signing in from a preview redirect lands on the dashboard, not the preview | The `next` parameter was dropped, and there are two honest reasons. Either the dashboard host and `preview_proxy_base` share no registrable parent — the session cookie cannot reach preview hostnames, so the bounce-back is refused on purpose; open the preview from the dashboard so the link carries a `_forge_token`. Or `preview_proxy_base` changed since the link was made, so the URL no longer names a preview under it. |
