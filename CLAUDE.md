...
| `internal/textfmt` | Small shared rendering helpers with no home of their own. `Count(n, noun)` ("1 quest" / "3 quests"), `Noun` and `Suffix` (the bare ""/"s" for `%d worker%s` format strings) are one package rather than five copies: pluralization had been re-derived in `cmd/forge`, `questgiver`, `web` and `daemon` (twice, once open-coded), in two shapes and under two names — none of them disagreeing, which is why the next surface wrote a sixth instead of finding one of the five |
