...
| `internal/selfdeploy` | Rebuilds and restarts the Forge daemon from source after a merge lands on Forge's own repository (config-gated). Deploys that end anywhere other than "new binary live and restarting" — drain timeout, failed swap, failed restart, rollback — are escalated through an injected `Emitter` into Hearth's Needs Attention list, since a rollback is otherwise invisible. The set the drain waits on is the daemon's `activeWorkerIDs`, which is work that owns a live process or pipeline goroutine (Smith, the quench/burnish/rebase/Assay fix workers) plus operator-paused workers — never a bellows per-PR monitor row (`state.WorkerStatus.IsMonitorOnly`, covering both `monitoring` and `detached`): those are non-terminal, so `ActiveWorkers` returns them, but they hold no PID and outlive any restart, and counting them meant one PR parked in the fix loop deferred every deploy for the full `max_drain_wait`. `forge status` reads the same function for its "waiting on N workers" detail, so the two never disagree |
