...
| `internal/executil` | Platform-specific process execution. Also owns `CleanGitEnv`/`StripGitEnv`/`IsGitRepoEnvVar` — the single strip set for git's repo-location env vars (`GIT_DIR`, `GIT_WORK_TREE`, `GIT_COMMON_DIR`, `GIT_INDEX_FILE`, `GIT_OBJECT_DIRECTORY`, `GIT_ALTERNATE_OBJECT_DIRECTORIES`, `GIT_NAMESPACE`, `GIT_GRAFT_FILE`, `GIT_SHALLOW_FILE`, `GIT_CEILING_DIRECTORIES`, `GIT_DISCOVERY_ACROSS_FILESYSTEM`), so every `git -C <path>` Forge runs is answered by that path and not by the repository an ambient GIT_DIR points at |
