...
| `internal/diff` | Shared unified-diff parsing/shaping primitives (size cap, generated-file filtering, changed-file extraction) used by both Warden and Assay. `AutoGeneratedPatterns` (`filter.go`) is the one definition of "generated file", and it names each lockfile individually because the obvious glob does not: `**/*.lock` matches Cargo/Gemfile/composer/`yarn.lock` but not `package-lock.json`, `pnpm-lock.yaml` or `npm-shrinkwrap.json`, which do not end in `.lock` — and matching yarn.lock is precisely what made the glob look like it worked while the largest lockfiles went through, so an anvil whose `assay.skip_paths` said `**/*.lock` was reviewing every one of them in full (one PR: 94.9% of its diff bytes were three `package-lock.json` files, $8.75). A per-anvil `skip_paths` list is for repo-specific noise on top of this, never for restating it. The failure is silent by construction — a glob that matches nothing filters nothing and the only symptom is the bill — so `TestAutoGeneratedPatternsMatchRealNames` pins every glob against the literal filenames the tools emit, and `FilterAutoGenerated`'s dropped-path list is surfaced rather than discarded: Assay's `elidedFilesSection` puts `3 files elided as generated: …` in the shared prompt head (so a lockfile-only PR does not read to a pass as an empty one, capped at 10 names since a repo-wide regeneration would otherwise put the bulk back as filenames) and the daemon's `Assay review completed` line carries `elided_files`/`elided_bytes` |
