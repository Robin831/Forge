...
| `internal/epic` | The one place the epic opt-in and the epic branch name are decided. `IsOrchestrated(labels)` is true only for the `crucible` label or an explicit `epic-branch:<name>` — a bead's issue type is deliberately not an input, so `issue_type: epic` alone no longer routes its children anywhere. `BranchName(id, labels)` is the single source of truth the poller (which stamps children with it) and the Crucible (which creates it) both call: they used to derive `epic/<id>` and `feature/<id>` independently, so a child dispatched outside the Crucible hard-failed on "base branch not found on origin" and burned the dispatch circuit breaker |
